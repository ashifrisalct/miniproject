...
include 'dbcon.php';
$t=$_REQUEST['t'];
$sql=mysqli_query($conn,"update registration set status='active' where acct_id='$t'");
$s1="select *from registration where acct_id='$t'";
$s2=mysqli_query($conn,$s1);
$e="";
$p="";
if($row=mysqli_fetch_assoc($s2))
{
$e=$row['email'];
$p=$row['password'];
}
$sql1=mysqli_query($conn,"insert into login values('$e','$p','user')");
if($sql>0)
	echo '<script>alert("User Has Been Approved Successfully");window.location.assign("member.php");</script>';
